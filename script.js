...
window.adminLogin = async function () {
  ...
  // FIX: declare btn here so it's accessible in finally
  const btn = document.querySelector("#adminForm button");
  if (btn) { btn.disabled = true; btn.textContent = "Logging in…"; }

  try {
    // Check if bus already has an active admin
    ...
    // FIX: sign in FIRST, then use the returned user object
    const userCredential = await signInWithEmailAndPassword(auth, email, pass);
    const user = userCredential.user;

    // Register this admin as active for the bus
    ...
    // FIX: start location tracking ONCE, after user is defined
...
window.downloadData = function () {
  ...
  triggerDownload(`attendance_${label}_${getTodayDate()}.csv`, "text/csv", csv);
};
...
window.downloadPDF = function () {
  ...
  pdfDoc.text(`RIT Bus Attendance — ${label}  (${getTodayDate()})`, 14, 15);
  pdfDoc.autoTable({
    startY: 22,
    head: [["S.no","Name","Reg No","Dept","Stop","Date","Time"]],
    body: data.map(r => [r.sno, r.name, r.regno, r.dept, r.stop, r.date, r.time]),
  });
  pdfDoc.save(`attendance_${label}_${getTodayDate()}.pdf`);
};

// ══════════════════════════════════════════════════════════
//  MANAGER PAGE  (placeholder — add your logic here)
// ══════════════════════════════════════════════════════════
function initManager() {
  // Manager page initialisation
}

// ══════════════════════════════════════════════════════════
//  MAIN DASHBOARD PAGE  (placeholder — add your logic here)
// ══════════════════════════════════════════════════════════
